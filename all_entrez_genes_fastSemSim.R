...
# Rscript all_entrez_genes_fastSemSim.R [<metric>]
...
outFold <- file.path("ALL_ENTREZ_GENES_FASTSEMSIM")
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "allEntrezGenesFastSemSim_logFile.txt")
if(!SSHFS) system(paste0("rm -f ", logFile))

all_gene_file <- file.path(setDir,
                           "/mnt/ed4/marie/entrez2synonym/entrez/ENTREZ_POS/gff_entrez_position_GRCh37p13_nodup.txt"
                           )
all_genes_DT <- read.delim(all_gene_file, header=T, stringsAsFactors = FALSE)
...
fastSemSim_queryFile <- file.path(outFold, "input", paste0("all_entrez_",fss_ontType, "_", fss_root, "_", fss_metric, "_all_entrezGenes.txt"))
system(paste0("mkdir -p ", dirname(fastSemSim_queryFile)))

fastSemSim_resultFile <- file.path(outFold, "output", paste0("all_entrez", "_", fss_ontType, "_", fss_root, "_", fss_metric, "_", fss_mixStrategy, "_result_file.txt"))
system(paste0("mkdir -p ", dirname(fastSemSim_resultFile)))

# if fss_ICtable == NULL => run to output the ICtable only
fss_ICtable <- file.path("ALL_ENTREZ_GENES_IC_TABLE/output", paste0("all_entrez_", fss_ontType, "_", fss_root, "_", fss_metric, "_",  fss_mixStrategy,"_IC_table.txt"))
fss_ICoutFile <- NULL
...
all_gene_entrez <- unique(as.character(all_genes_DT$entrezID))

cat("# of genes retrieved from entrez table\t=\t", length(all_gene_entrez), "\n")

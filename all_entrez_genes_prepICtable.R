...
startTime <- Sys.time()
cat(paste0("> Rscript all_entrez_genes_prepICtable.R\n"))

# Rscript all_entrez_genes_prepICtable.R [<metric>]
# Rscript all_entrez_genes_prepICtable.R Resnik
...
outFold <- file.path("ALL_ENTREZ_GENES_IC_TABLE")
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "allEntrezGenesICtable_logFile.txt")
...
fss_ICtable <- NULL
fss_ICoutFile <- file.path(outFold, "output", paste0("all_entrez_",fss_ontType, "_", fss_root, "_", fss_metric, "_", fss_mixStrategy, "_IC_table.txt"))
